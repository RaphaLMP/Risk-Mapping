...
const Layout = ({ children }) => {
  ...
  const youtubeVideoId = "0"; // Substitua pelo ID real do vídeo

  // Componente para um membro da equipe (inline)
  const TeamMember = ({ name, role, imageSrc, linkedinUrl, githubUrl }) => (
    <div className="text-center text-gray-500"> {/* Removi dark:text-gray-400 */}
      <img className="mx-auto mb-4 w-36 h-36 rounded-full" src={imageSrc} alt={`${name} Avatar`} />
      <h3 className="mb-1 text-2xl font-bold tracking-tight text-gray-900"> {/* Removi dark:text-white */}
        <a href="#">{name}</a>
      </h3>
      <p>{role}</p>
      <ul className="flex justify-center mt-4 space-x-4">
        <li>
          <a href={linkedinUrl} className="text-[#39569c] hover:text-gray-900" aria-label={`LinkedIn de ${name}`}> {/* Removi dark:hover:text-white */}
        ...
        <li>
          <a href={githubUrl} className="text-gray-900 hover:text-gray-900" aria-label={`GitHub de ${name}`}> {/* Removi dark:hover:text-white dark:text-gray-300 */}
    ...
    <div className="w-full max-w-screen-xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 mb-4 flex justify-center">
      <div className="relative w-1/2" style={{ paddingTop: '28.125%' }}>
        <iframe
          ...
          src={`https://www.youtube.com/embed/${videoId}`} // Use o formato de embed do YouTube
  ...
  // Componente para a seção "Nosso Time" (inline)
  const TeamSection = ({ members }) => (
    <section className="bg-white py-8 px-4 mx-auto max-w-screen-xl text-center lg:py-16 lg:px-6"> {/* Removi dark:bg-gray-900 */}
  <div className="mx-auto mb-8 max-w-screen-sm lg:mb-16">
    <h2 className="mb-4 text-4xl tracking-tight font-extrabold text-gray-900"> {/* Removi dark:text-white */}
      Nosso Time
    </h2>
  </div>
  ...
  // Componente para a seção "Um mapa vivo feito por todos" (inline)
  const LiveMapSection = () => (
    <section className="bg-white py-8 px-4 mx-auto max-w-screen-xl text-center lg:py-16 lg:px-6 mb-8"> {/* Removi dark:bg-gray-900 */}
      <div className="mx-auto mb-8 max-w-screen-sm lg:mb-16">
        <h2 className="mb-4 text-4xl tracking-tight font-extrabold text-gray-900"> {/* Removi dark:text-white */}
          Um mapa vivo feito por todos
        </h2>
        <p className="font-light text-gray-500 sm:text-xl text-justify"> {/* Removi dark:text-gray-400 */}
        ...
        <section>
          <h2 className="mb-4 text-4xl tracking-tight font-extrabold text-gray-900 mt-8"> {/* Removi dark:text-white */}
            Veja o vídeo
          </h2>
